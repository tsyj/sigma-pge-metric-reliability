...
sys.path.insert(0, "/data/xinyuan/GOAI_ai4s_env/scripts"); sys.path.insert(0, "/data/xinyuan/GOAI_ai4s_env/e52")
from env2_merid import ENV, LEDGER
...
def main():
    ...
    Q26, S26, QFL = [], [], []
    for rid, _ in r26:
        q = base_quantities(f"{ENV}/runs_merid/{rid}")
        if q:
    ...
    P26, PFL = [], []
    for a, b in PAIRS:
        qa, qb = base_quantities(f"{ENV}/runs_merid/{a}"), base_quantities(f"{ENV}/runs_merid/{b}")
    ...
    json.dump(dict(n_candidates=len(rows), n_pass_A=len(near), n_pass_AB=len(ok),
              ...
              open(f"{ENV}/ledger/metric_search_merid.json", "w"),
              ensure_ascii=False, indent=1)
    print(f"\n→ ledger/metric_search_merid.json")
