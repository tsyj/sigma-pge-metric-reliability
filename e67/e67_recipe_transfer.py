#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""E67：E66 的配方选出的尺子，换强迫方向还站得住吗。预注册 sha 见 PREREG_E67.sha256。"""
...
log('经向…');  Mr=axes(f'{ENV}/ledger/env2_merid_runs.jsonl',  f'{ENV}/runs_merid',  'hidden',  False)
log('  %d 个候选  (%.0fs)'%(len(Mr),time.time()-t0))
log('45°…');   D4=axes(f'{ENV}/ledger/env2_diag45_runs.jsonl', f'{ENV}/runs_diag45', 'hidden',  False)
...
def passes(T,k): 
    v=T.get(k); return bool(v and v['A']>=0.7 and v['B']>=0.9)
def rate(T,S):
    if not S: return None,0
    n=sum(1 for k in S if passes(T,k)); return n/len(S), n
def base_rate(T):
    if not T: return 0.0
    return sum(1 for v in T.values() if v['A']>=0.7 and v['B']>=0.9)/len(T)

res=dict(prereg_sha=open(f'{ENV}/e67/PREREG_E67.sha256').read().strip(),
         n_zonal=len(Z), n_merid=len(Mr), n_diag45=len(D4),
         size_S_A=len(S_A), size_S_AP=len(S_AP))
for tag,T in (('merid',Mr),('diag45',D4)):
    br=base_rate(T)
    rA,nA=rate(T,S_A); rP,nP=rate(T,S_AP)
    res[tag]=dict(base_rate=round(br,4),
      ...
      S_AP=dict(n=nP, rate=round(rP,4) if rP is not None else None,
               expect=round(br*len(S_AP),1), enrich=round(nP/(br*len(S_AP)),2) if br*len(S_AP)>0 else None))
tri_A =[k for k in S_A  if passes(Mr,k) and passes(D4,k)]
tri_AP=[k for k in S_AP if passes(Mr,k) and passes(D4,k)]
res['triple']=dict(S_A=len(tri_A), S_AP=len(tri_AP))
surv=[k for k in S_AP if passes(Mr,k)]
...
res['prereg_check']={
 'P1 S_A⊥ 跨向存活率 > S_A': (bool(rP>rA) if (rA is not None and rP is not None) else None),
 'P2 两集合富集均 ≥2 倍': bool((res['merid']['S_A']['enrich'] or 0)>=2 and (res['merid']['S_AP']['enrich'] or 0)>=2),
 'P3 存活者全为比值型': res['merid_survivors_all_ratio'],
 'P4 三向交集 S_A⊥ > S_A': bool(len(tri_AP)>len(tri_A)),
}
res['elapsed_s']=round(time.time()-t0,1)
json.dump(res,open(f'{ENV}/e67/E67_RECIPE_TRANSFER.json','w'),indent=1,ensure_ascii=False)
log('')
