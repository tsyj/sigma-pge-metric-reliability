...
print('diag45...',flush=True); Mr=wind_passlist(f'{ENV}/ledger/env2_diag45_runs.jsonl',f'{ENV}/runs_diag45','hidden')
...
uv=('u|all|rms','v|all|mean_abs'); vu=('v|all|rms','u|all|mean_abs'); dp=('u|d800|rms',None); dpv=('v|d800|rms',None)
...
res=dict(zonal_pass=len(zp),merid_pass=len(mp),tide_alarm=len(tp),deep_u_diag45=Mr.get(dp),deep_v_diag45=Mr.get(dpv),merid_tide_list=[dict(num=k[0],den=k[1]) for k in mt],
...
json.dump(res,open('/data/xinyuan/GOAI_ai4s_env/e55/E55_CROSS.json','w'),indent=1,default=str)
