...
LEDGER = f"{ENV}/ledger/env2_diag45_runs.jsonl"
...
FIXED = {
    ...
    "grid_hv": "48×48×13",
    "forcing": "E55 转 45°: 风应力 τ=0.1 N/m²，前 ~12 h tanh 爬升",
...
def _truth(day, akv=1e-5):
    ...
    if d not in _TRUTH_CACHE:
        _TRUTH_CACHE[d] = {round(i * DT_MIT / 86400, 3): (read_field("U", i, d), read_field("V", i, d))
                           for i in iters("U", d)}
...
class SigmaPGEEnv:
    ...
    def _diagnose(self, rd, bathy, ok, ntimes=8640, akv=1e-5):
        ...
        if U is not None:
            ...
            zr = roms_depths(Cs, h)
            UU, VV = U
            uz = to_z(rho_from_u(u), zr, _rc_truth()); vz = to_z(rho_from_v(v), zr, _rc_truth())
            m = min(uz.shape[-2], UU.shape[-2], vz.shape[-2], VV.shape[-2]); n = min(uz.shape[-1], UU.shape[-1], vz.shape[-1], VV.shape[-1])
            A = np.concatenate([uz[:, :m, :n].ravel(), vz[:, :m, :n].ravel()]); B = np.concatenate([UU[:, :m, :n].ravel(), VV[:, :m, :n].ravel()])
