...
def cell(r,c):
    m=r
    if c==0:  # A1 风驱配对抗刷
        return {'uv_ratio':(P,'1.00'),'deep_dc_rms':(D,'0.000 (deep_rms_800)'),'temp_d400':(B,'抗刷 1.00, 排序 ρ=+0.85 反向'),'Pnet_MW':(NA,'仅内潮')}[m]
    if c==1:  # A2 平底
        return {'Pnet_MW':(P,'−0.0000 MW'),'deep_dc_rms':(B,'0.018 (平底本无伪流)'),'temp_d400':(B,'平底 4.000 / 海山 3.98–4.00'),'uv_ratio':(D,'分母→0 发散')}[m]
    if c==2:  # A3 盒宽
    ...
    if c==4:  # A5 预注册
        return {'uv_ratio':(D,'P3 预测失效→被反驳'),'deep_dc_rms':(B,'P2 预测排不出序, 实测 ρ=−0.9'),'Pnet_MW':(P,'P5 穿零→中'),'temp_d400':(NA,'无预测')}[m]
    if c==5:  # A6 跨环境
        return {'uv_ratio':(D,'纬向✓ 内潮✓ 经向 ρ=+0.17'),'deep_dc_rms':(D,'三处排序 |ρ|≈0.99, B 全败'),'temp_d400':(D,'内潮满分, 风驱 ρ=+0.85'),'Pnet_MW':(NA,'仅内潮')}[m]
    if c==6: return a7(m)
    if c==7:  # A8 真值锚
