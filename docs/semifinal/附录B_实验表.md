...
| E59 | 无真值特征预测排得准的程度（预注册） | 15376 训练(纬向) + 15376 测试(经向) | 跨环境 AUC=0.871；单特征 f1 抗糊弄的程度 AUC=0.2005（负向）、f7 旋钮秩相关 0.6752；P3 未命中：precision@204=0.0882 < 基率 0.2797216441207076；命中率非单调，峰值 0.8306 后回落至 0.6906（最极端 0.5% 为 0.052）；两条事后解释（方向不可测、外推失败）均被数据推翻并留档 | e59/PREREG_E59.md（sha 533c3062…）、e59/E59_TRUTHFREE.json、E59B_POSTHOC.json |
