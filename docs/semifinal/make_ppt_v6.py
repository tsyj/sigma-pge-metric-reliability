...
pic(s,FIG2+'fig_pareto.png',0.6,1.6,8.4)
txt(s,9.2,1.75,3.9,5.4,14.5,DARK,line=['• 三重交集 0，与随机期望 0.71 无差别——"0"不是证据；证据是贫化与死因','• 转风向：u/v 排序 −0.92→+0.17；转成 v/u 排序回来但抗刷只剩 0.11','• 45° 预注册：四条预测两条被反驳，机理降为描述性陈述','• 自家论文头条也死：环带功率只改盒宽摆 7.24×，修一行时钟 4.58→7.89——论文将更正'])
# p5 Agent 三角色（表）
