...
# p4 主结果 E52 图通栏
s=prs.slides.add_slide(blank); head(s,'主结果：风向一转，两环境交集的 13 把全部死亡','把尺子跟着转成 v/u——排序回来了（−0.92），但换平底就能刷（抗刷率 0.11）：排序与抗刷分了家')
...
# p6 沉淀
s=prs.slides.add_slide(blank); head(s,'交的是协议，不是尺子','137 条算例一次完赛、md5 审计；1 条预测被反驳、1 次指标降格、1 次论文更正、1 次环境 bug 声明')
