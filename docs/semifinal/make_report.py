...
CAP_ENV='''图 1　探索环境四元组 (K, V, H, A)。K 是 Agent 可拧的旋钮，V 是公开读数（Agent 只看得到这一层），H 是保密判分（风驱侧为 MITgcm z 坐标独立求解器真值，内潮侧为 500 km 自参照锚，共用平底与静止态两条按构造零真值的通道），A 是九个审计算子。与文献中「把评测器藏起来」的现行做法相比，本环境的区别在于把 A 提为一等公民，并且把它同时砍向 V 与 H 自身——A3 只改盒宽就让隐藏判分侧的环带功率摆 7.24 倍，A4 修一行初值时钟让它从 4.58 变 7.89 MW。'''
...
while i<len(lines):
    ...
    if cm and cm.group(1) in CAPFIG:
        ...
        i+=1; continue
    para(ln,10.5)
    if 'v6' in SRC and ln.startswith('## 2.1 '):
        fig(FIG2+'fig_env.png',CAP_ENV,15.5)
    if 'v6' in SRC and ln.startswith('## 4.8 统计地板'):
        fig(FIG2+'fig_truthfree.png',CAP_TF,14.0); fig(FIG2+'fig_asym.png',CAP_ASYM,15.0)
