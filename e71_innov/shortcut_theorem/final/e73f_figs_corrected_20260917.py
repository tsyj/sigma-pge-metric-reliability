...
# 2026-09-17更正：原脚本444保留；本派生脚本只改标题/图注/版面/输出名，数据与判分不动。
QUAL = '限定：经向与45°为同一批22个run的四旋钮单变扫描（VISC2 7、VISC4 7、TNU2 7、AKV_BAK 1）。\n22条真值对应19个独立配置（4条同默认配置重复）；N=15376名义候选；与multienv共用45°，非独立。\n【2026-09-17复算更正，原图停用】'
...
fig.tight_layout(rect=(0, 0.11, 1, 0.89)); fig.savefig(f"{FIN}/FIG1_partial_AB_zonal_corrected_20260917.png", dpi=200, bbox_inches="tight"); plt.close(fig)
...
ax.set_title("45° 反号象限：符号场由 sign(c_45·D_z) 预测\nP2 命中率 %.4f（n=%d，门槛 0.90，中）；c_45=%.4f（n_45=%d 条真值，19个独立配置）" % (m["P2"]["hit_rate"], m["P2"]["n_conf"], c45, m["n_45"]), fontsize=10.5, loc="left")
...
fig.tight_layout(rect=(0, 0.11, 1, 1)); fig.savefig(f"{FIN}/FIG2_sign_quadrant_45_corrected_20260917.png", dpi=200, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(rect=(0, 0.11, 1, 0.95)); fig.savefig(f"{FIN}/FIG3_residual_board_45_corrected_20260917.png", dpi=200, bbox_inches="tight"); plt.close(fig)
print("FIGS_DONE")
