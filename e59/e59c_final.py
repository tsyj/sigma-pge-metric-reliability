...
res['operational_reading']=('无真值特征对排序力有实质预测力（跨环境 AUC=%.3f，中段命中率峰值 %.3f vs 基率 %.3f），'
...
ax.text(cx[0],base+0.018,'随机基率 %.3f'%base,fontsize=11,color=GRAY)
ax.plot(cx,cy,'-o',color=BLUE,lw=2.4,ms=8,zorder=4)
ax.plot(cx[peak],cy[peak],'o',ms=15,mfc='none',mec=AMBER,mew=2.6,zorder=5)
ax.annotate('峰值 %.3f\n(score≈%.3f, n=%d)'%(cy[peak],cx[peak],cn[peak]),xy=(cx[peak],cy[peak]),
...
ax.annotate('预测分最高的一箱\n命中率反而只有 %.3f'%cy[-1],xy=(cx[-1],cy[-1]),
            xytext=(cx[-1]*0.42,cy[-1]+0.30),fontsize=11,color=RED,ha='center',
            arrowprops=dict(arrowstyle='->',color=RED,lw=1.4))
ax.set_xscale('log'); ax.set_xlabel('无真值预测分（纬向风上训练，经向风上评估；对数轴）')
ax.set_ylabel('该箱内真实通过排序力判据的比例')
ax.set_ylim(-0.03,1.0)
ax.set_title('无真值预测器有效，但它自己也在极端处失效\n跨环境 AUC = %.3f；命中率升到 %.2f 后跌回 %.2f'%(
