...
if $PY e44_tide/scorecard.py 2>/dev/null; then
  echo "  -> 已重绘 e44_tide/figs/fig_killboard_print.png 与 fig_killboard_slide.png"
else
  echo "  [跳过重绘] 需要 matplotlib 与中文字体；随包已带成品图，下面的数值结论不受影响"
fi
echo "== 2/4 参照系表重算 =="
...
print('  BH93: u_max %.2f->%.2f (rho=%+.3f); A1 抗糊弄 %.3f; 代理夸大 %.2f 倍; 技巧全程为负 %s'%(
...
print('  跨环境 AUC=%s (纬向训练->经向测试); f1 抗糊弄的程度单特征 AUC=%s'%(R['multi']['auc_test'],R['single']['f1 配对抗糊弄的程度']['auc_test']))
