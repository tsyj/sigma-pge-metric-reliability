...
FAILED=0
step(){ if ! "$@"; then FAILED=$((FAILED+1)); echo "  [FAIL] $*" >&2; fi; }
...
_sc_err=$(mktemp 2>/dev/null || echo "${TMPDIR:-/tmp}/scorecard_err.$$")
if $PY e44_tide/scorecard.py 2>"$_sc_err"; then
  echo "  -> 已重绘 e44_tide/figs/fig_killboard_print.png 与 fig_killboard_slide.png，并重写 e44_tide/analysis/KILLBOARD.json"
else
  echo "  [跳过重绘] scorecard.py 未跑完（常见原因：缺 matplotlib 或中文字体）；随包已带成品图与 KILLBOARD.json，下面的数值结论不受影响"
  echo "  [跳过原因] $(grep -v '^[[:space:]]*$' "$_sc_err" | tail -1)"
fi
rm -f "$_sc_err"
echo "== 2/4 参照系表重算 =="
step bash baselines/run_baselines.sh
...
# 2026-09-17：保留复算行为，交集为零不作为泛化不可能的证据。
print('  历史交集/零假设期望（仅对账，不支持不可能性结论）: 纬∩经 %d/%.1f  纬∩潮 %d/%.1f  经∩潮 %d/%.1f  三重 %d/%.2f (P0=%.2f)'%(
...
print('  BH93: u_max %.2f->%.2f (rho=%+.3f); A1 刷不动 %.3f; 代理夸大 %.2f 倍; 技巧全程为负 %s'%(
  B['bh93_umax'][0],B['bh93_umax'][-1],B['rho_bh93'],B['A1_paired_correct'],B['exaggeration'],B['skill_always_negative']))
PYX
then FAILED=$((FAILED+1)); echo '  [FAIL] 主结论重算块' >&2; fi
echo "== 3b/4 E59 使用无真值特征的预筛（读取历史判定） =="
if ! $PY - <<'PYX'
import json
R=json.load(open('e59/E59_TRUTHFREE.json'))
print('  跨环境 AUC=%s (纬向训练->经向测试); f1 刷不动的程度单特征 AUC=%s'%(R['multi']['auc_test'],R['single']['f1 配对抗刷率']['auc_test']))
...
print('  8 局均值: 简单线性模型 %.3f > 只看单条线索的土办法 %.3f > Agent %.3f > 随机 %.3f'%(S['logistic'],S['b_heuristic'],S['agent'],S['random']))
print('  符号检验 p: Agent>随机 %.4f (不显著); Agent>简单线性模型 %.2f'%(S['p_agent_gt_random_sign'],S['p_agent_gt_logistic_sign']))
PYW
then FAILED=$((FAILED+1)); echo '  [FAIL] 主结论重算块' >&2; fi
echo "== 4/4 自检（与 smoke_test 同源） =="
step bash scripts/smoke_test.sh
if [ "$FAILED" -eq 0 ]; then
  echo "REPRODUCE_CORE: DONE (0 failed)"
else
  echo "REPRODUCE_CORE: FAILED ($FAILED step(s) failed)" >&2
  exit 1
fi
