...
FIXED = {
    ...
    "grid_hv": "48×48×13",
    "forcing": "纬向风应力 τ=0.1 N/m²，前 ~12 h tanh 爬升",
...
TRUTH_BY_AKV = {
    1e-5: f"{ENV}/truth/hires_1pEm5",
    1e-4: f"{ENV}/truth/sw_viscAz_1pEm4",
    1e-3: f"{ENV}/truth/hires_1pEm3",
    1e-2: f"{ENV}/truth/sw_viscAz_1pEm2",
    1e-1: f"{ENV}/truth/sw_viscAz_1pEm1",
    1.0:  f"{ENV}/truth/sw_viscAz_1p0",
}
_RC = None
...
def _truth(day, akv=1e-5):
    ...
    d = TRUTH_BY_AKV.get(min(TRUTH_BY_AKV, key=lambda z: abs(np.log10(z) -
                                                             np.log10(max(akv, 1e-12)))))
    if d not in _TRUTH_CACHE:
        _TRUTH_CACHE[d] = {round(i * DT_MIT / 86400, 3): read_field("U", i, d)
                           for i in iters("U", d)}
...
class SigmaPGEEnv:
    ...
    def _diagnose(self, rd, bathy, ok, ntimes=8640, akv=1e-5):
        ...
        if U is not None:
            ...
            zr = roms_depths(Cs, h)
            uz = to_z(rho_from_u(u), zr, _rc_truth())
