...
def s1():
    ...
    print("  一共列出了 %d 把候选尺子。" % n)
    print()
    print("    %s%6d 把" % (pad("能把方法的好坏排对的", 36), a))
    print("    %s%6d 把" % (pad("在「什么都没修」的对照上不乱说的", 36), b))
    print("    %s%6d 把" % (pad("两样都做到的", 36), both))
    exp = a * b / n
    print()
    print("  如果这两件事互不相干，两样都做到的该有 %.0f 把。实际只有 %d 把。" % (exp, both))
    print("  也就是说：排得准的那批，恰恰最容易在对照上乱说。")
    print("  而真实的科研里我们没有答案，只看得见它排得准。")
    print()
    rec = json.load(open("e67/E67B_RECIPE_TRANSFER.json", encoding="utf8"))
    ok = rec["n_zonal"] == n and rec["size_S_A"] == both
    print("  这个 %d，在看数据之前就写下来封存了。刚才是重算的 —— %s。"
...
def s3():
    ...
    print()
    print("  换四种陡度的海山 —— 在对照上不乱说的那批指标：")
    print("      合格比例 %s" % (" / ".join("%.1f%%" % (x * 100) for x in f)))
    print("      两两重合度 %.2f – %.2f  → 几乎是同一批" % (min(j), max(j)))
    print()
    print("  → 要判断「排得准不准」，得先有一个更可信的参照；本例换成经向风就反了")
    print("  → 对照检验在测过的四种地形上比较稳，但它替代不了「排得准不准」这一关")
...
SCREENS = [
    ("── 1/5  一万五千多把候选尺子，两样都做到的只有 204 把 ──────────", s1),
    ("── 2/5  新旧指标的相互评判 ──────────────────────────────────", s2),
